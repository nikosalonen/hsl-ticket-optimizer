...
/**
 * Determines the user's system color-scheme preference.
 *
 * @returns `"dark"` if the system prefers a dark color scheme, `"light"` otherwise.
 */
...
/**
 * Apply the selected color theme to the document and update the theme toggle icons.
 *
 * @param theme - The theme to apply, either `"light"` or `"dark"`. Sets `data-theme` on the root element and updates visibility of the theme icons accordingly.
 */
...
/**
 * Initialize the application's color theme and wire theme-related UI and system listeners.
 *
 * Loads a saved theme from localStorage or falls back to the system preference, applies it,
 * attaches a click handler to the theme toggle control that switches and persists the user-selected theme,
 * and listens for system color-scheme changes to update the theme only when the user has not set an explicit preference.
 */
function initTheme() {
...
/**
 * Reads a CSS custom property from the document root and returns its computed color value.
 *
 * @param cssVar - The CSS variable name to read (for example, `--color-primary`)
 * @param alpha - Optional alpha value between 0 and 1 to apply to the returned color
 * @returns The computed color string for the given CSS variable; if `alpha` is provided it is injected into the color expression so the returned value includes the specified transparency
 */
...
/**
 * Injects provided HTML into the results content container and reveals the results area, triggering the entrance animation if it was previously hidden.
 *
 * @param html - HTML markup to render inside the results content element
 */
...
/**
 * Clears the results content and hides the results container.
 *
 * Removes any rendered HTML inside the results area and toggles the container to a hidden, non-animated state.
 */
...
/**
 * Convert a raw zone input into an API-ready zone code.
 *
 * @param rawValue - The user-entered zone string (may include letters A–D or other text)
 * @returns The API zone code derived from `rawValue` when it contains only letters A–D (case-insensitive); otherwise the trimmed input string
 */
...
/**
 * Render the comparison UI for ticket pricing options.
 *
 * Builds the HTML for the results section, including a highlighted "Best Option" stat and a grid of cards
 * for each available ticket option (monthly/annual costs, savings vs most expensive, and calculation details).
 *
 * @param result - Computed pricing outcome containing cost, annual cost, calculation text, and the optimal option key for each ticket type
 * @returns The HTML string for the results section ready to be injected into the DOM
 */
...
/**
 * Create or update a bar chart comparing monthly costs for available ticket options.
 *
 * Replaces any existing cost comparison chart instance and renders monthly cost bars for
 * Single, Season, Continuous and optionally Series 10 and Series 20 if present in `result`.
 *
 * @param result - Pricing result containing monthly costs for `single`, `season`, `continuous`, and optional `series10`/`series20`
 */
...
/**
 * Renders or updates the "Trips per week" line chart showing monthly cost comparisons across ticket options.
 *
 * Renders the chart into the canvas with id "trips-cost-chart" and replaces any existing chart instance.
 *
 * @param baseTripsPerWeek - The central trips-per-week value used to build the range of x-axis values around it.
 * @param baseOptions - Base monthly cost values and optional series ticket metadata used to compute costs:
 *   - `single`: monthly cost when paying per trip
 *   - `season`: monthly cost of a season ticket
 *   - `continuousMonthly` (optional): monthly cost of a continuous subscription
 *   - `series10` / `series20` (optional): objects with `price`, `journeys`, and `validityDays` for series ticket types
 */
...
/**
 * Creates a debounced version of a function that delays invoking `fn` until after `ms` milliseconds have elapsed since the last call.
 *
 * @param fn - The function to debounce
 * @param ms - Delay in milliseconds to wait before invoking `fn`
 * @returns A function that schedules `fn` to run after `ms` milliseconds; calling it again before the delay elapses resets the timer
 */
...
/**
 * Fetches pricing for the current form inputs, determines the optimal ticket option, and updates the UI with results and charts.
 *
 * Reads selected zones, trips-per-week, and municipality from the DOM; when zones are not selected it clears results and exits silently.
 * Manages loading and error state, requests filtered ticket prices, computes the best option, renders the comparison HTML, and updates the cost and trips charts.
 */
