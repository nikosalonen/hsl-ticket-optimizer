...
export class PriceService {
  ...
  async fetchTicketPrices(
    ...
    try {
      ...
      void this.pingSaverSubscriptionEndpoint(
        zones,
        customerGroup,
        homemunicipality,
      );
      ...
      if (seasonResponse.tickets && Array.isArray(seasonResponse.tickets)) {
        const monthlyTicket = seasonResponse.tickets.find(
          (ticket) =>
            ...
            ticket.zones === parseInt(zones, 10) &&
      ...
      if (
        seasonResponse.subscriptions &&
        Array.isArray(seasonResponse.subscriptions)
      ) {
        const cont = seasonResponse.subscriptions.find(
          (s) =>
            s.customerGroup === customerGroup &&
            s.zones === parseInt(zones, 10) &&
  ...
  calculateContinuousMonthlyTicketCost(
    ...
    const annualCost =
      Math.round(monthlyCost * PriceService.MONTHS_PER_YEAR * 100) / 100;
    const calc =
      continuousMonthlyPrice && continuousMonthlyPrice > 0
  ...
  findOptimalOption(
    ...
    const single = this.calculateSingleTicketCost(
      tripsPerWeek,
      prices.single,
      summerVacation,
    );
    const season = this.calculateSeasonTicketCost(
      prices.season,
      summerVacation,
    );
    ...
    const series10 = prices.series10
      ? this.calculateSeriesTicketCost(
          tripsPerWeek,
          prices.series10,
          summerVacation,
        )
